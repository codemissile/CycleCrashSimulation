...
namespace Gadd420
{
    ...
    public class RB_Controller : MonoBehaviour
    {
        ...
        public bool enableStoppiesBETA = true;
        ...
        float ogWheelieTorque;
        public float stoppieTorque = 1500f;
        ...
        void CalulateAcceleration()
        {
            // Calculate the change in velocity divided by the change in time
            currentAcceleration = (rb.velocity - previousVelocity) / Time.fixedDeltaTime;
            previousVelocity = rb.velocity;
        }

        //method to get the current acceleration
        ...
        void HandleBrakes()
        {
            ...
            if (inputs.FrontBreakInput>0 && inputs.combineBrakeInputs == false)
            {
                //Brake torque set
            ...
            if(inputs.combineBrakeInputs == true)
            {
                if(inputs.VInput < 0)
                {
                    wheelColliders[1].brakeTorque = frontBrakeTorque;
        ...
        void Stoppies()
        {
            if (inputs.WheelieInput == 1 && inputs.FrontBreakInput == 1)
            {
                wheelieTorque = stoppieTorque;
